...
async def mod_accepted_handler(callback_query: CallbackQuery, callback_data: dict):
    ...
    post_accepted_info = MESSAGES['post_sent_to_channel'].format(post_text=data['post_text'])
...
async def mod_denied_handler(callback_query: CallbackQuery, callback_data):
    ...
    edit_request_text = MESSAGES['edit_post_request'].format(post_text=data['post_text'])
